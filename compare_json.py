...
def compare_properties(arcgis_file, fulcrum_file):
    ...
    # Extracting properties from the first layer in ArcGIS data
    arcgis_properties = set()
    if "layers" in arcgis_data and len(arcgis_data["layers"]) > 0:
        arcgis_properties = set(arcgis_data["layers"][0].keys())

    # Extracting properties from the first feature in Fulcrum data
    fulcrum_properties = set()
    if "layers" in fulcrum_data and len(fulcrum_data["layers"]) > 0:
        fulcrum_properties = set(fulcrum_data["layers"][0].keys())
